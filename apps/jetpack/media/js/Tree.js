...
Tree = Class.refactor(Tree, { options: { editable: true } });

Collapse = Class.refactor(Collapse, {
    ...
    addBranch: function(attr, target){
        attr = Object.merge(Object.clone(this.options.branch), attr);
        attr.html = '<a class="expand" id="' + attr.id + '" href="#"></a><span title="' + attr.title + '">' + attr.title + '</span>' + ((attr.rel == 'directory') ? '<ul></ul>' : '');

        var li = new Element('li', attr).inject((target) ? $(target) : this.element);
    ...
    addPath: function(obj, options){
        var options = options || {},
            ...
            id = options.id || '',
            levels = obj.path.split('/'),
            end = levels.length - 1,
            selector = '';

        levels.each(function(name, i, a){
            if(i == end){
